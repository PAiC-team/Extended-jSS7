                ...
                case _ID_maxTransferredVolume:
                    if (!asnInputStream.isTagPrimitive())
                        throw new CAPParsingComponentException("Error while decoding " + _PrimitiveName
                                ...
                                CAPParsingComponentExceptionReason.MistypedParameter);
                    this.maxTransferredVolume = (long) asnInputStream.readIntegerData(length);
                    break;
                case _ID_maxElapsedTime:
                    if (!asnInputStream.isTagPrimitive())
                        throw new CAPParsingComponentException("Error while decoding " + _PrimitiveName
                                ...
                                CAPParsingComponentExceptionReason.MistypedParameter);
                    this.maxElapsedTime = (int) asnInputStream.readIntegerData(length);
                    break;
    ...
    public void encodeData(AsnOutputStream asnOutputStream) throws CAPException {
        ...
        try {
            if (this.maxTransferredVolume != -1) {
                asnOutputStream.writeIntegerData(this.maxTransferredVolume);
            } else {
                asnOutputStream.writeIntegerData(this.maxElapsedTime);
            }
        } catch (IOException e) {
