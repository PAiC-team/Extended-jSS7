        ...
        if (asnInputStream.getTagClass() == Tag.CLASS_CONTEXT_SPECIFIC) {
            switch (tag) {
                case _ID_timeGPRSIfNoTariffSwitch:
                    if (!asnInputStream.isTagPrimitive())
                        throw new CAPParsingComponentException("Error while decoding " + _PrimitiveName
                                ...
                                CAPParsingComponentExceptionReason.MistypedParameter);
                    this.timeGPRSIfNoTariffSwitch = (int) asnInputStream.readIntegerData(length);
                    break;
                case _ID_timeGPRSIfTariffSwitch:
                    ...
                    ((TimeGPRSIfTariffSwitchImpl) this.timeGPRSIfTariffSwitch).decodeData(asnInputStream, length);
                    break;
                default:
    ...
    public void encodeData(AsnOutputStream asnOutputStream) throws CAPException {
        ...
        try {
            if (this.timeGPRSIfNoTariffSwitch != null) {
                asnOutputStream.writeIntegerData(timeGPRSIfNoTariffSwitch.intValue());
            } else {
