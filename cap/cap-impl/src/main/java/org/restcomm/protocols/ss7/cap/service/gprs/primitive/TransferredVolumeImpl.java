        ...
        if (asnInputStream.getTagClass() == Tag.CLASS_CONTEXT_SPECIFIC) {
            switch (tag) {
                case _ID_volumeIfNoTariffSwitch:
                    if (!asnInputStream.isTagPrimitive())
                        throw new CAPParsingComponentException("Error while decoding " + _PrimitiveName
                                ...
                                CAPParsingComponentExceptionReason.MistypedParameter);
                    this.volumeIfNoTariffSwitch = asnInputStream.readIntegerData(length);
                    break;
                case _ID_volumeIfTariffSwitch:
                    ...
                    ((VolumeIfTariffSwitchImpl) this.volumeIfTariffSwitch).decodeData(asnInputStream, length);
                    break;
    ...
    public void encodeData(AsnOutputStream asnOutputStream) throws CAPException {
        ...
        try {
            if (this.volumeIfNoTariffSwitch != null) {
                asnOutputStream.writeIntegerData(volumeIfNoTariffSwitch.longValue());
            } else {
                ((VolumeIfTariffSwitchImpl) this.volumeIfTariffSwitch).encodeData(asnOutputStream);
            }
        } catch (IOException e) {
