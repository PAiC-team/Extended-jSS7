    ...
    public void registerMBean(MBeanLayer layer, MBeanType type, String name, Object bean) {
        if (mBeanServer != null) {
            try {
                ...
                logger.error(
                        String.format("Error while trying to register MBean for Layer=%s Type=%s name=%s", layer, type, name),
                        e);
            }
        } else {
    ...
    public Object unregisterMBean(MBeanLayer layer, MBeanType type, String name) {
        ...
        if (mBeanServer != null) {
            try {
                ...
                logger.error(
                        String.format("Error while trying to unregister MBean for Layer=%s Type=%s name=%s", layer, type, name),
                        e);
            }
        } else {
            logger.error(String.format("mBeanServer is null. Cannot register MBean for Layer=%s Type=%s name=%s", layer, type,
